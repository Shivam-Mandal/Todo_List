        ...
        <div className="bg-black/20 backdrop-blur-md rounded-xl p-8 shadow-xl max-w-md w-full text-white">
          ...
          <ul className="space-y-4">
            {todos.map((todo) => (
              <li
                key={todo.id}
                className={`flex justify-between items-center px-4 py-2 rounded-lg shadow backdrop-blur-md ${todo.done ? "bg-green-500/30" : "bg-white/20"
                  }`}
              >
                {editingId === todo.id ? (
                  <>
                  ...
                  <>
                    <div
                      className={`flex-1 text-left ${todo.done ? "line-through text-gray-200" : ""
                        }`}
                    >
                      {todo.text}
                    </div>
